...
const SETTING = 'twitchAlerts';
const POLL_NOTE = 'Saved. Alerts post when the streamer goes live — background polling runs when the bot is hosted with network access.';

module.exports = {
  category: 'notifications',
  data: new SlashCommandBuilder()
    .setName('twitch-alerts')
    .setDescription('Manage Twitch live alert subscriptions.')
    .addSubcommand((s) =>
      s.setName('add').setDescription('Subscribe to a Twitch channel')
        .addStringOption((o) => o.setName('channel_url').setDescription('Twitch channel URL or name').setRequired(true))
        .addChannelOption((o) => o.setName('discord_channel').setDescription('Where to post alerts').addChannelTypes(ChannelType.GuildText)))
    .addSubcommand((s) =>
      s.setName('remove').setDescription('Unsubscribe from a Twitch channel')
        .addStringOption((o) => o.setName('channel_url').setDescription('Twitch channel URL or name').setRequired(true)))
    .addSubcommand((s) => s.setName('list').setDescription('List Twitch alert subscriptions'))
  ...
  async execute(interaction, client) {
    ...
    // list
    if (alerts.length === 0) {
      return interaction.reply({ embeds: [embed.info('No Twitch alert subscriptions configured.', '🟣 Twitch Alerts')], ephemeral: true });
    }
    const lines = alerts.map((a, i) => `**${i + 1}.** ${a.source} → <#${a.channelId}>`);
    return interaction.reply({ embeds: [embed.base().setTitle('🟣 Twitch Alerts').setDescription(lines.join('\n'))] });
  },
};
